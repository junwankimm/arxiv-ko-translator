...
- **Sentinel absent** (first-ever run on this machine) → **automatically run the
  `arxiv-ko-setup` skill** (invoke it via the Skill tool) — this is the default; don't make
  the user ask. Briefly tell them: "First run — installing the LaTeX/Korean toolchain." Use
  `arxiv-ko-setup`'s **default no-sudo path** (user-tree TeX Live in `~/texlive`), which needs
  **no password**, so the whole install is hands-off. Once setup writes the sentinel, continue
  to Step 1. (Only fall back to the sudo BasicTeX route if the user prefers the faster install.)
